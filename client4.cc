...
#include <omnetpp.h>
#include "CTSGenerator4.h"  // Include the header file

using namespace omnetpp;

class client4 : public cSimpleModule {
private:
    int messageCount;
    int maxMessageCount;
...
void client4::handleMessage(cMessage *msg) {
    EV << "client4 handle message" << endl;
    ...
    if (strcmp(msg->getName(), "Request") == 0) {
        ...
        if (receivingQueue.getLength() >= 50) {
            // If the receivingQueue is full, drop the incoming message immediately
            EV << "Dropping incoming message from another client4 because the receivingQueue is full" << endl;
            droppedMessageCount++;
            delete msg;
        } else {
...
void client4::sendNextMessage() {
    messageCount++;

    cMessage *msg = generateCTSMessage4();

    const char encryptionKey = 'K';
    const std::string& originalMessage = msg->getName();
    std::string encryptedMessage = "SM4:" + std::to_string(messageCount) + ":" + originalMessage;
...
void client4::handleDroppedMessage(cMessage *msg) {
    if (droppedMessageRequest) {
        const std::string& encryptedMessage = droppedMessageRequest->getName();
        cMessage *resendMessage = new cMessage(encryptedMessage.c_str());
        resendMessage->setContextPointer(droppedMessageRequest->getContextPointer());
...
void client4::transferMessageFromReceivingQueue() {
    if (!receivingQueue.isEmpty()) {
