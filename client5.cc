...
#include <omnetpp.h>
#include "CTSGenerator5.h"  // Include the header file

using namespace omnetpp;

class client5 : public cSimpleModule {
private:
    int messageCount;
    int maxMessageCount;
...
void client5::initialize() {
    EV << "client5 initialize" << endl;
...
void client5::handleMessage(cMessage *msg) {
    EV << "client5 handle message" << endl;
    ...
    if (strcmp(msg->getName(), "Request") == 0) {
        ...
        if (receivingQueue.getLength() >= 50) {
            // If the receivingQueue is full, drop the incoming message immediately
            EV << "Dropping incoming message from another client5 because the receivingQueue is full" << endl;
            droppedMessageCount++;
            delete msg;
        } else {
...
void client5::sendNextMessage() {
    messageCount++;

    cMessage *msg = generateCTSMessage5();

    const char encryptionKey = 'K';
    const std::string& originalMessage = msg->getName();
    std::string encryptedMessage = "SM5:" + std::to_string(messageCount) + ":" + originalMessage;
...
void client5::handleDroppedMessage(cMessage *msg) {
    if (droppedMessageRequest) {
        const std::string& encryptedMessage = droppedMessageRequest->getName();
        cMessage *resendMessage = new cMessage(encryptedMessage.c_str());
        resendMessage->setContextPointer(droppedMessageRequest->getContextPointer());
...
void client5::transferMessageFromReceivingQueue() {
    if (!receivingQueue.isEmpty()) {
