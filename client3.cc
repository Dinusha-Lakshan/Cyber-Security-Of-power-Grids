...
#include <omnetpp.h>
#include "CTSGenerator3.h"  // Include the header file

using namespace omnetpp;

class client3 : public cSimpleModule {
private:
    int messageCount;
    int maxMessageCount;
...
void client3::initialize() {
    EV << "client3 initialize" << endl;
...
void client3::handleMessage(cMessage *msg) {
    EV << "client3 handle message" << endl;
    ...
    if (strcmp(msg->getName(), "Request") == 0) {
        ...
        if (receivingQueue.getLength() >= 50) {
            // If the receivingQueue is full, drop the incoming message immediately
            EV << "Dropping incoming message from another client3 because the receivingQueue is full" << endl;
            droppedMessageCount++;
            delete msg;
        } else {
...
void client3::sendNextMessage() {
    messageCount++;

    cMessage *msg = generateCTSMessage3();

    const char encryptionKey = 'K';
    const std::string& originalMessage = msg->getName();
    std::string encryptedMessage = "SM3:" + std::to_string(messageCount) + ":" + originalMessage;
...
void client3::handleDroppedMessage(cMessage *msg) {
    if (droppedMessageRequest) {
        const std::string& encryptedMessage = droppedMessageRequest->getName();
        cMessage *resendMessage = new cMessage(encryptedMessage.c_str());
        resendMessage->setContextPointer(droppedMessageRequest->getContextPointer());
...
void client3::transferMessageFromReceivingQueue() {
    if (!receivingQueue.isEmpty()) {
